 ...
 */
public enum FieldTypeToByteEnum {
    /**
     * mysql列类型
     */
    ...
    FieldTypeToByteEnum(byte code, Function<Proto, Object> parseByte) {
        this.code = code;
        this.parseByte = parseByte;
    }

    public static FieldTypeToByteEnum parse(byte code) {
        for (FieldTypeToByteEnum value : values()) {
    ...
    public static FieldInfo makeFieldInfo(String dbName, String tableName, String tableRealName, Class<?> clazz, int index, String fieldName) {
        if (Number.class.isAssignableFrom(clazz)) {
            return new FieldInfo(dbName, tableName, tableRealName, fieldName, fieldName, 0, index, FieldTypeEnum.FIELD_TYPE_FLOAT, (short) 0, (byte) 0);
        } else if (String.class.isAssignableFrom(clazz)) {
            return new FieldInfo(dbName, tableName, tableRealName, fieldName, fieldName, 0, index, FieldTypeEnum.FIELD_TYPE_VARCHAR, (short) 0, (byte) 0);
        } else {
