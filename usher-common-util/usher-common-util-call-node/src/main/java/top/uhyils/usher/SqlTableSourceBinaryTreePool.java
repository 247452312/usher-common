...
public class SqlTableSourceBinaryTreePool extends AbstractObjectPool<SqlTableSourceBinaryTreeInfo> {
    ...
    public static SqlTableSourceBinaryTreePool getInstance() {
        return SqlTableSourceBinaryTreePoolHolder.instance;
    }
    ...
    public SqlTableSourceBinaryTreeInfo getOrCreateObject(SQLExprTableSource tableSource, List<SQLBinaryOpExpr> where) {
        ...
        if (name instanceof SQLPropertyExpr) {
            ...
            owner = sqlPropertyExpr.getOwner() != null ? sqlPropertyExpr.getOwnernName() : callerUserInfo.getDatabaseName();
        } else if (name instanceof SQLIdentifierExpr) {
            owner = callerUserInfo.getDatabaseName();
        }
    ...
    private static class SqlTableSourceBinaryTreePoolHolder {

        private static final SqlTableSourceBinaryTreePool instance = new SqlTableSourceBinaryTreePool();
    }
}
